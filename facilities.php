<!DOCTYPE html>
<html lang="en">

<head>
    <link rel="stylesheet" href="./styles/facilities.css">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FACILITIES</title>
</head>
<?php include 'header.php'; ?>

<body>
    <main>

        <div class="title">
            <h1>FACILITIES</h1>
        </div>
        <div class="card-container">

            <div class="card">
                <img src="./styles/images/FACILITIES/lobby.webp">
                <div class="intro">
                    <h1>Lobby</h1>
                    <p>Main entrance where parents, staff, and visitors enter and exit the school; students picked up and dropped off as needed.</p>
                </div>
            </div>

            <div class="card">
                <img src="./styles/images/FACILITIES/prinicipal.webp">
                <div class="intro">
                    <h1>Admin Office</h1>
                    <p>Oversees academic standards, curriculum, assessment, instruction, and research across all District schools.</p>
                </div>
            </div>

            <div class="card">
                <img src="./styles/images/FACILITIES/central.webp">
                <div class="intro">
                    <h1>Central Office</h1>
                    <p>Supports school logistics and promotes student health and safety.
                    </p>
                </div>
            </div>

            <div class="card">
                <img src="./styles/images/FACILITIES/faculty.webp">
                <div class="intro">
                    <h1>Faculty Office</h1>
                    <p> Space for educators to relax, eat, collaborate, plan lessons, and discuss educational matters.</p>
                </div>
            </div>



            <div class="card">
                <img src="./styles/images/FACILITIES/board.webp">
                <div class="intro">
                    <h1>Board Room</h1>
                    <p>Large table for board members to discuss ideas and address audiences during meetings.</p>
                </div>
            </div>

        </div>





    </main>

    <?php include 'footer.php'; ?>

</body>

</html>